...
module.exports = {
  async create(req, res){
    ...
    try {
      ...
      const id_person = await knex('person').insert({name, surname, born_at, nr_document, gender});
      console.log(id_person);
      await knex('users').insert({id_user, id_group_users, id_person, user_name, password:password_hash});

      await knex('employee').insert({id_employee, id_user})
